...
export interface StatsCardProps {
  ...
  onBarClick?: (item: ChartDataItem, index: number) => void;
  onBarHover?: (item: ChartDataItem | null, index: number | null) => void;
}

// Sub-component for animating the numerical value
export const AnimatedValue = ({ value, prefix = "", postfix = "" }: { value: number; prefix?: string; postfix?: string }) => {
        ...
        <CardContent>
          <div ref={cardRef} className="flex flex-col gap-4">
            ...
            <motion.div
              ...
              onMouseLeave={() => onBarHover?.(null, null)}
            >
              {chartData.map((item, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    (onBarClick || onBarHover) && "cursor-pointer"
                  )}
                  onClick={() => onBarClick?.(item, index)}
                  onMouseEnter={() => onBarHover?.(item, index)}
                >
                  <motion.div
                    className={cn(
